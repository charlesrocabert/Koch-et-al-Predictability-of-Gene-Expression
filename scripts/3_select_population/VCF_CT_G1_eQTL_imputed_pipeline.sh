...
python ./local/2_FilterGenotypes.py -repository-path $REPOSITORY_PATH -gatk-path $GATK_PATH -population CT_G1 -version Tcas3.30 -imputed -suffix eQTL_imputed
python ./local/3_VariantsToTable.py -repository-path $REPOSITORY_PATH -gatk-path $GATK_PATH -population CT_G1 -version Tcas3.30 -suffix eQTL_imputed -annotation
